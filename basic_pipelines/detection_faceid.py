...
def app_callback(pad, info, user_data):
    ...
    if frame is not None and detection_records:
        embeddings = user_data.face_pipeline.get_embeddings(frame, detection_records)
        if detection_records and not embeddings:
            print(f"[face-id-debug] frame {frame_num}: all {len(detection_records)} detections filtered before tracking")
    ...
    print(
        f"Frame {frame_num:5d} | FPS: {fps_str:5s} | People detected: {detection_count:2d} | IDs: [{ids_str}]"
    )
...
if __name__ == "__main__":
    ...
    os.environ.setdefault(
        "GST_PLUGIN_FEATURE_RANK", "kmssink:NONE,glimagesink:NONE,waylandsink:NONE"
    )
